...
const DetailCVScreen = ({ navigation }) => {
  ...
  const isButtonDisabled = () => {
    if (!faceImage || !bodyImage || !ktpImage || !selectedPekerjaan || !selectedPendidikan || !pendidikan || !status || !tinggiBadan || !beratBadan || !selectedIbadah || !kriteria || !deskripsi || !hobi || !anak || !selectedSuku || !selectedWarnaKulit || !penyakit || !organisasi || !kelebihan || !kekurangan || !aktivitasHarian || !visimisi || !firstQA || !secondQA || !!thirdQA) {
      return true
    } else {
      return false
    }
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
      ...
      <Row style={{ marginVertical: 8 }}>
        <Touchable style={{ flex: 1, marginRight: 8 }} onPress={() => {
          setSelectType('face')
          setImagePickerVisible(true)
        }}>
          <Card style={{ alignItems: 'center' }}>
            <Image
              source={faceImage ? { uri: `data:image/png;base64,${faceImage}` } : IMAGES_RES.man_head}
              style={{ height: 152, margin: 4, borderRadius: 4, width: 152 }}
              resizeMode={'contain'}
            />
            <Text>Upload Foto Wajah</Text>
          </Card>
        </Touchable>
        <Touchable style={{ flex: 1, marginRight: 8 }} onPress={() => {
          setSelectType('body')
          ...
          <Card style={{ alignItems: 'center' }}>
            <Image
              source={bodyImage ? { uri: `data:image/png;base64,${bodyImage}` } : IMAGES_RES.man_body}
              style={{ height: 152, margin: 4, borderRadius: 4, width: 152 }}
              resizeMode={'contain'} />
            <Text>Upload Foto Full</Text>
          </Card>
        </Touchable>
      </Row>
      <Touchable onPress={() => {
        ...
        <Card style={{ height: 148, alignItems: 'center', justifyContent: 'center' }}>
          {!ktpImage ? <Text>Upload Foto KTP</Text> : <Image
      ...
      <View style={styles.child}>
        ...
        <Text
          style={styles.textInfo}>Sebutkan dan jelaskan minimal 3 sifat antum</Text>
        <Input
        ...
        <Text
          style={styles.textInfo}>Contoh: Pagi-Sore Bekerja dsb, semakin detail semakin baik</Text>
        <Input
      ...
      <Modal
        ...
        onOptionButtonPress={(type) => type == 'first' ? loadFromGallery() : loadFromCamera()} />
    </ScrollView>
  );
};
